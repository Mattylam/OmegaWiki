...
conditions: "Requires (a) a programmatically-scorable validation metric capturing task quality, (b) a small training set (tens to hundreds of labelled examples), (c) budget for offline compilation (multiple LLM calls per training example during bootstrapping). Benchmarks are closed-domain with verifiable answers; extension to open-ended or preference-based tasks is not directly tested."
